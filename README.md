...
- `id`: 영문 소문자, 숫자, 하이픈으로 만든 고유 ID. 생성되는 파일명이 `{id}.json`이 됩니다.
- `name`: 화면에 표시할 종목명
- `ticker`: 종목 코드
...
휴대폰에서 선택한 종목 정보는 외부에서 볼 수 없습니다. 목록에 없는 종목을 추가하고 싶다면 **종목명, 종목 코드, 뉴스 검색어**를 알려 주세요. 위 스크립트로 등록한 뒤 뉴스 수집과 배포까지 진행할 수 있습니다.
...
- `config/stocks.json`이 `main`에 올라오면 즉시 실행
